...
def animate(n):
    ...
    cont = plt.plot(x, Temp_old, color='xkcd:salmon', label='Current Temperature profile')  # Contour plotting the values
    ...
    plt.pause(0.01) # do not do this while animating! memory overload
    return cont,

ani = anim.FuncAnimation(fig, animate, interval = 50, frames=501)  # Invoke the animation function.
# Pass the animate function as an argument.
# Interval sets the time period between the frames in milli-second
# fig is the figure handle
# Set the number of frames needed. This data set has 49 time snaps!
# Hence, set frames = 499
...
# To save the video
# fps = frames per second. Choose a value based on frames on top!
